            ...
            for p in ParameterGrid(parameter_values):
                try:
                    ...
                    results_df.loc[len(results_df)] = [ models_to_run[index], clf, p, outcomes,
                                                        ...
                                                        pipeline.get_precision(y_test, score, 1.0),
                                                        pipeline.get_precision(y_test, score, 0.01),
                                                        pipeline.get_precision(y_test, score, 0.02),
                                                        pipeline.get_precision(y_test, score, 0.05),
                                                        pipeline.get_precision(y_test, score, 0.1),
                                                        pipeline.get_precision(y_test, score, 0.2),
                                                        pipeline.get_precision(y_test, score, 0.3),
                                                        pipeline.get_precision(y_test, score, 0.5),
                                                        pipeline.get_recall(y_test, score, 0.01),
                                                        pipeline.get_recall(y_test, score, 0.02),
                                                        pipeline.get_recall(y_test, score, 0.05),
                                                        pipeline.get_recall(y_test, score, 0.1),
                                                        pipeline.get_recall(y_test, score, 0.2),
                                                        pipeline.get_recall(y_test, score, 0.3),
                                                        pipeline.get_recall(y_test, score, 0.5),
